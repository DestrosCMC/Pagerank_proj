...
vector = gensim.downloader.load("glove-twitter-100")
...
class WebGraph():
  ...
  def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            # The row vector where a[i]==1 iff row i of P is all zeros
            a = torch.zeros(n)
        ...
        if query != '':
            ...
            for i, url in enumerate(urls):
                for word,simscore in rl:
                    scores[i] += url.count(word) * simscore**50
        else:
...
def related_words(term):
    ...
    if len(related_list) > 5:
            related_list = related_list[:5]
    words = [pair[0] for pair in related_list]
    return words

def related_words_and_score(term):
    related_list = vector.most_similar(term, topn=5)
    #if len(related_list) > 5:
            #related_list = related_list[:5]
